...
// boss
//
// Modifications by: PST/Wolf3DGuy
//
#ifndef SPEAR
			case 214:
...
//
    ...
    if (shadetoggle)
    {
        if (gamestate.mapon != FINALMAP)
            LevelShadeTableBuilder(0,0,0);
        else
            LevelShadeTableBuilder(25,0,0);
    }
    else
    {
        if (playstate != ex_died)
            BuildShadeTable (0,0,0);
    }
#else
    if (playstate != ex_died)
        BuildShadeTable (0,0,0);
...
#ifdef WEAPONC
	gamestate.nextweapon = -1;		// take away weapon
#else
...
//
	...
	if (gamestate.lives > -1)
	{
		...
		gamestate.rockets = gamestate.fuel = 0; gamestate.ammo = STARTAMMO;
        if (!godflag)
	...
	do
	{
        ...
        else if (!quadflag)
        {
            if (!gamestate.quadmode)
                godmode = gamestate.quadmodecount = 0;
        }
...
#ifdef SPEAR
		if (gamestate.mapon == FINALMAP)	// give them the key always //God dammit, them Id Guys can't spell? me too, Id Guys, me too...
		{
			gamestate.keys |= 1;
			DrawKeys ();
		}
...
startplayloop:
		...
		case ex_secretlevel:
...
#ifdef NEEDLEDEMO
			if ( (gamestate.mapon == SECRET3) || (gamestate.mapon == 5 && playstate != ex_secretlevel) )
			{
				died = true;			// don't "get psyched!"
			...
			// GOING TO SECRET LEVEL
			//
			// Changed the Macros cuz to make life easier :3
			//
			if (playstate == ex_secretlevel)
				switch(gamestate.mapon)
				{
				 case FROMSECRET1: gamestate.mapon = SECRET3; break;
				 case FROMSECRET2: gamestate.mapon = SECRET2; break;
				}
			//
			// COMING BACK FROM SECRET LEVEL
			//
			else if (gamestate.mapon >= SECRET1 && gamestate.mapon < TESMAP)
				switch(gamestate.mapon)
				{
#ifndef NEEDLEDEMO
				 case SECRET3: gamestate.mapon = FROMSECRET1+1; break;
#else
                 case SECRET3: break;
#endif
				 case SECRET2: gamestate.mapon = FROMSECRET2+1; break;
				}
#endif
			else
			//
