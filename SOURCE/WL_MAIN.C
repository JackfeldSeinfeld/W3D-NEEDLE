...
void ReadConfig(void)
{
	...
	//
		...
		read(file,&message_toggle,sizeof(message_toggle));
		read(file,&fancyweaps,sizeof(fancyweaps));
		read(file,&shadetoggle,sizeof(shadetoggle));
	...
	//
		...
		if (!message_toggle) message_toggle = true;
		if (!fancyweaps) fancyweaps = true;
		if (!shadetoggle) shadetoggle = true;
		if (viewsize != 15) viewsize = 15;
...
void WriteConfig(void)
{
	...
	if (file != -1)
	{
		...
		write(file,&message_toggle,sizeof(message_toggle));
		write(file,&fancyweaps,sizeof(fancyweaps));
		write(file,&shadetoggle,sizeof(shadetoggle));
...
void NewGame (int difficulty,int episode)
{
	...
	// Mercy Change: if you're playing on Bite Me! difficulty, you now have 12 bullets
    //               now it's 8 in hard and extreme difficulties... and 20 in easy and baby ones...  
	if (gamestate.difficulty >= gd_hard)
      gamestate.ammo = STARTAMMO_OG;
    else if (gamestate.difficulty <= gd_easy)
      gamestate.ammo = STARTAMMO_EASY;
	else
	...
	if (gamestate.difficulty >= gd_hard)
      ...
      gamestate.lives = 5;
	gamestate.maxammo = gamestate.maxfuel = 100;
    ...
    if (!godflag)
      ...
      quadmode = gamestate.quadmode = false;
}
...
//
...
#else
			DEATHSCREAM6SND,		33,		// FLAPJACKSCREAM.WAV
#endif
...
#else
            YEAHSND,			    38,		// Got Gat replacement
#endif
			...
			NEINSOVASSND,           20,
			DOGATTACKSND,           21,
			MANSSIGHTSND,			29,		// Mans Sight
			MANSDEATHSND,			30,		// Mans Death
			WILHELMSIGHTSND,		31,		// Wilhelm Sight
			WILHELMDEATHSND,		32,		// Wilhelm Death
			SAKURAIDEATHSND,		33,		// Sakurai Death
			...
			SATISFYINGSND,          40,     // Gibbing
			MISSILEFIRESND,         41,     // Self Explanatory...
			MISSILEHITSND,          42,     // Also Self Explanatory...
			FLAMETHROWERSND,        43,     // Can we Stop with it?
			OPENWDOORSND,           44,
			CLOSEWDOORSND,          45,
			SPEARFALLSND,           46,     // Jeez, Again?
			GETPOWSND,              47,
			BACHTUNGSND,            48,     // Friedrich Brent Sight
			SCHAISERSND,            49,     // Friedrich Brent Death
			HEALTH0SND,             50,     // Actually just N from Murder Drones
			SAKURAISIGHTSND,        51,     // BANZAIIIIIII!!!
#ifndef NEEDLEDEMO
			BITEMESND,              52,     // BITE ME!!
#endif
#endif
#endif
			LASTSOUND
...
#else
			CA_CacheGrChunk (CREDITSPIC1);
			VWB_DrawPic (0,0,CREDITSPIC1);
			UNCACHEGRCHUNK (CREDITSPIC1);

			CA_CacheGrChunk (CREDITSPIC2);
			VWB_DrawPic (0,80,CREDITSPIC2);
			UNCACHEGRCHUNK (CREDITSPIC2);
#endif
			VH_UpdateScreen();
			VW_FadeIn ();
...
#else
		if (Keyboard[sc_Tab] && MS_CheckParm("uziwashere"))
#endif
#endif
			RecordDemo ();
		else
