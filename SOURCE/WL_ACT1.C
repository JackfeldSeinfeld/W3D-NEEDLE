...
{SPR_STAT_52,bo_flamethrower},  // FLAME THROWER!
{SPR_STAT_53,bo_rocketlauncher},// ROCKET LAUNCHER!
{SPR_STAT_54,bo_backpack},      // backpack
{SPR_STAT_55,bo_fuel},          // fuel canister
{SPR_STAT_56,bo_rockets},       // rocket crate
#else
{SPR_STAT_48,bo_flamethrower},// FLAME THROWER!
{SPR_STAT_49,bo_rocketlauncher},// ROCKET LAUNCHER!
#endif
{SPR_BRICKWINDOW_2,bl_dir_south},
{SPR_BRICKWINDOW_1,bl_dir_east},
{SPR_STAT_57,block},			// Table with Mugs spr2v
{SPR_GOD_BALL_POW,bo_god},      // God Mode Power Up
{SPR_QUAD_DMG_POW,bo_quad},     // Quad Damage Power Up
...
void SpawnSecret (int tilex, int tiley)
{
    laststatobj->shapenum = SPR_NOTHING;
    ...
    laststatobj->flags = FL_BONUS;
    laststatobj->itemnumber = bo_secret;
...
//
	...
	if (areabyplayer[area])
	{
        if (lock != dr_wooden)                        
		    ...
		    PlaySoundLocTile(CLOSEWDOORSND,doorobjlist[door].tilex,doorobjlist[door].tiley);	// JAB
	}
...
void OperateDoor (int door)
{
	...
	if (lock >= dr_lock1 && lock <= dr_lock2)
	{
		if ( ! (gamestate.keys & (1 << (lock-dr_lock1) ) ) )
		{
			...
			GetMessage("You need a key here...");
			return;
		}
	}
	...
	//
		...
		if (areabyplayer[area1])
		{
            if (lock != dr_wooden)
		        ...
		        PlaySoundLocTile(OPENWDOORSND,doorobjlist[door].tilex,doorobjlist[door].tiley);	// JAB
		}
	}
