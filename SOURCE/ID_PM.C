...
=====================
=
= PM_GetFlatTexturePage
=
= As PM_GetPage, but takes a flat texture page index in BX
=
=====================
*/

memptr PM_GetFlatTexturePage (void)
{
	memptr	result;
    int     pagenum;
	...
	{
		if (!(result = PML_GetPageFromXMS(pagenum,false)))
		{
			if (PMPages[pagenum].lastHit == PMFrameCount)
				PMThrashing++;

			PML_LoadPage(pagenum,false);
