...
#else
        if (ob->obclass == spectreobj || ob->obclass == spearobj)
#endif
...
//
	...
	if (ob->hitpoints<=0)
	{
        if ( damage > 100 && gamestate.chosenweapon != wp_knife ) 
            //ob->flags |= DTH_GIB;
            gamestate.gibbed = true;
		KillActor (ob);
    }
	else
	{
...
//
	...
	case mansobj:
		SD_PlaySound(MANSSIGHTSND);
		NewState (ob,&s_manschase1);
		ob->speed = 1800;			// go faster when chasing player
		break;

#ifdef NEEDLE
	case uberobj:
		SD_PlaySound(SAKURAISIGHTSND);
        NewState (ob,&s_uberchase1);
		ob->speed = 2750;			// go faster when chasing player
		break;
#else
	case uberobj:
		NewState (ob,&s_uberchase1);
	...
	case friedobj:
		SD_PlaySound(BACHTUNGSND);
		NewState (ob,&s_friedchase1);
		ob->speed = 2500;			// go faster when chasing player
        break;
